...
class PracticeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ([
                     ...
                     "Les globaux sont mauvais"=> "Les globaux sont mauvais. Les fonctions sont meilleures que les types. Les objets sont probablement meilleurs que les structures de données complexes.",
                     "Types intégrés"=> "L'utilisation des types intégrés de Python - et de leurs méthodes - sera plus rapide que l'écriture de vos propres types (sauf si vous écrivez en C). Si les performances sont un facteur important, essayez de trouver un moyen d'utiliser les types intégrés standard plutôt que des objets personnalisés.",
                     "Injection de dépendances"=> "L'injection de dépendances est un modèle de codage utile pour être clair sur ce que sont vos dépendances et d'où elles viennent. (Faites en sorte que les objets, les méthodes, etc. reçoivent leurs dépendances en tant que paramètres plutôt que d'instancier eux-mêmes de nouveaux objets). Cela rend les signatures d'API plus complexes, c'est donc un compromis. Si vous vous retrouvez avec une méthode qui nécessite 10 paramètres pour toutes ses dépendances, c'est un bon signe que votre code en fait trop, de toute façon. L'article définitif sur l'injection de dépendances est \"Inversion of Control Containers and the Dependency Injection Pattern\", par Martin Fowler.",
                     "Peu de simulations"=> "Plus vous devez faire des simulations pour tester votre code, plus votre code est mauvais. Plus vous devez instancier et mettre en place du code pour pouvoir tester un comportement spécifique, plus votre code est mauvais. L'objectif est de créer de petites unités testables, ainsi que des tests d'intégration et fonctionnels de plus haut niveau pour vérifier que les unités coopèrent correctement.",
                     "API externes"=> "Les API externes sont celles pour lesquelles la \" conception en amont \" - et la prise en compte des cas d'utilisation futurs - est vraiment importante. Changer d'API est pénible pour nous et pour nos utilisateurs, et créer une incompatibilité rétroactive est horrible (bien que parfois impossible à éviter). Concevez les API externes avec soin, tout en respectant le principe \" les choses simples doivent rester simples \".",
                     "30 lignes max"=> "Si une fonction ou une méthode dépasse 30 lignes de code, envisagez de la décomposer. Une bonne taille maximale de module est d'environ 500 lignes. Les fichiers de test ont tendance à être plus longs que cela.",
                     "Pas de job dans les constructeurs"=> "Ne faites pas de travail dans les constructeurs d'objets, qui sont difficiles à tester et surprenants. Ne mettez pas de code dans __init__.py (sauf les importations pour l'espacement des noms). __init__.py n'est pas l'endroit où les programmeurs s'attendent généralement à trouver du code, donc c'est \"surprenant\".",
